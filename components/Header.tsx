      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex items-center group cursor-pointer">
            ...
            <span className="ml-2 text-2xl font-bold lowercase text-black tracking-tight">zysculpt</span>
          </div>
          ...
          <div className="hidden lg:flex items-center gap-3">
            <button className="px-5 py-2.5 text-sm font-semibold text-gray-700 hover:text-black transition-colors">
              Log In
            </button>
            <button className="primary-btn px-6 py-2.5 rounded-full text-sm font-semibold">
              Create account
            </button>
          </div>
        ...
        <div className="lg:hidden bg-white border-t border-gray-100 py-4 px-4 space-y-4 animate-in slide-in-from-top-2">
          {navData.map((nav, index) => (
            <div key={index} className="space-y-2">
              <div className="font-bold text-gray-900 px-2">{nav.label}</div>
              <div className="grid grid-cols-1 gap-1 pl-4">
                {nav.items.map((item, idx) => (
                  <a key={idx} href="#" className="py-2 text-sm text-gray-500 hover:text-[#1918f0]">
                    {item}
                  </a>
                ))}
              </div>
          ...
          <div className="pt-4 flex flex-col gap-3">
            <button className="w-full py-3 text-center font-semibold text-gray-700 border border-gray-200 rounded-xl">
              Log In
            </button>
            <button className="w-full primary-btn py-3 text-center font-semibold rounded-xl">
              Create account
            </button>
          </div>
        </div>
